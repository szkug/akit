...
import com.korilin.akit.publics.AndroidAsyncImageContextData
import com.korilin.akit.publics.AsyncImageContext
import com.korilin.akit.publics.PainterModel
import com.korilin.akit.publics.ResIdModel
...
internal abstract class GlideRequestNode(
    ...
    private fun RequestBuilder<Drawable>.requestFlow(requestModel: RequestModel): Flow<GlideLoadResult<Drawable>> {
        return when (val model = requestModel.model) {
            is Int -> return flowOfId(androidData.context, model)
            is File -> load(model)
            is Uri -> load(model)
    ...
    private suspend fun flowRequest(requestModel: RequestModel) {
        androidData.requestBuilder(androidData.context)
            .setupSize()
