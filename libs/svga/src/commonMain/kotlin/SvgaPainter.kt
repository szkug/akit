...
internal class SvgaPainter(
    private val prepared: PreparedSvgaMovie,
    private val state: SvgaPlayerState,
    ...
    override fun DrawScope.onDraw() {
        ...
        if (movie.width <= 0f || movie.height <= 0f) return
        val frameIndex = state.currentFrame.coerceIn(0, movie.frames.coerceAtLeast(1) - 1)
        ...
        val active = movie.sprites.mapNotNull { sprite ->
            sprite.frames.getOrNull(frameIndex)?.let { frame -> SvgaSpriteSnapshot(sprite, frame) }
        }
        var index = 0
        while (index < active.size) {
            ...
            if (matteKey.isNullOrBlank()) {
                drawSprite(snapshot, movieMatrix, frameIndex, hitRegions, BlendMode.SrcOver)
                index += 1
                continue
            }
            ...
            val matte = active.firstOrNull { it.sprite.imageKey == matteKey || normalizeBitmapAssetKey(it.sprite.imageKey) == normalizeBitmapAssetKey(matteKey) }
            if (matte == null) {
                group.forEach { drawSprite(it, movieMatrix, frameIndex, hitRegions, BlendMode.SrcOver) }
                continue
            }
            drawIntoCanvas { canvas ->
                canvas.saveLayer(groupBounds(group + matte, movieMatrix), androidx.compose.ui.graphics.Paint())
            }
            group.forEach { drawSprite(it, movieMatrix, frameIndex, hitRegions, BlendMode.SrcOver) }
            drawSprite(matte, movieMatrix, frameIndex, hitRegions, BlendMode.DstIn)
    ...
    private fun DrawScope.drawSprite(
        ...
        movieMatrix: androidx.compose.ui.graphics.Matrix,
        frameIndex: Int,
        ...
        withTransform({
            ...
            val drawBlock: DrawScope.() -> Unit = {
                when {
                    ...
                    dynamicEntity.texts[key] != null -> drawDynamicText(frame.layout, dynamicEntity.texts.getValue(key), frame.alpha, blendMode)
    ...
    private fun computeMovieMatrix(movieWidth: Float, movieHeight: Float, canvasSize: Size): androidx.compose.ui.graphics.Matrix {
