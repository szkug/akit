...
@JvmInline
private value class CoilBinaryEngineContext(val context: PlatformContext) : BinaryEngineContext

private val CoilBinaryEngineContextProvider: BinaryEngineContextProvider =
    { CoilBinaryEngineContext(LocalPlatformContext.current) }

val BinaryEngineContext.platformContext get() = (this as CoilBinaryEngineContext).context
...
class CoilBinaryRequestEngine(
    ...
    companion object {
        val Normal = CoilBinaryRequestEngine()

        init {
            LocalBinaryEngineContextRegister.register(
                CoilBinaryRequestEngine::class,
                CoilBinaryEngineContextProvider,
            )
        }
    }
}
