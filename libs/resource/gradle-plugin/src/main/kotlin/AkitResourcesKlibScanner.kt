...
class KlibIrUsageScanner(
    ...
    fun scan(klibToolPath: String?, klibFiles: Set<File>, buildDir: File): UsedResources {
        if (klibToolPath.isNullOrBlank()) {
            logger.info(MunchkinResourcesMessages.PRUNE_SKIP_KLIB_TOOL_MISSING)
            return emptyUsage()
        }
        val tool = File(klibToolPath)
        if (!tool.exists()) {
            logger.info(MunchkinResourcesMessages.pruneSkipKlibToolAt(tool.absolutePath))
            return emptyUsage()
        }
        val existingKlibs = klibFiles.filter { it.exists() }.toSet()
        if (existingKlibs.isEmpty()) {
            logger.info(MunchkinResourcesMessages.PRUNE_SKIP_NO_KLIB_FILES)
            return emptyUsage()
        }
        logger.info(MunchkinResourcesMessages.pruneScanStart(existingKlibs.size))

        val tempRoot = File(buildDir, MunchkinResourcesConstants.KLIB_TEMP_DIR)
        if (tempRoot.exists()) {
    ...
    private fun scanKlib(
        ...
        val process = ProcessBuilder(
            tool.absolutePath,
            MunchkinResourcesConstants.KLIB_COMMAND_DUMP_IR,
        ...
        process.inputStream.bufferedReader().useLines { lines ->
            lines.forEach { line ->
                val fileMatch = MunchkinResourcesRegex.IR_FILE.find(line)
                if (fileMatch != null) {
                    currentFile = fileMatch.groupValues[1].trim()
                }
                if (MunchkinResourcesRegex.IR_FUN.containsMatchIn(line)) {
                    updateFunctionStack(line, funStack)
                }

                val signatureMatch = MunchkinResourcesRegex.IR_SIGNATURE.find(line)
                ...
                val propertyMatch = MunchkinResourcesRegex.IR_PROPERTY.find(line)
                ...
                val directMatch = MunchkinResourcesRegex.IR_DIRECT.find(line)
                if (directMatch != null) {
                    if (isResFile(currentFile)) return@forEach
                ...
                if (stringIds.isNotEmpty()) {
                    val anyPropertyMatch = MunchkinResourcesRegex.IR_ANY_PROPERTY.find(line)
        ...
        if (exit != 0) {
            logger.info(MunchkinResourcesMessages.pruneDumpIrFailed(klibPath.absolutePath, exit))
        }
    }
    ...
    private fun toKlibArchive(input: File, tempRoot: File): File? = when {
        input.isFile && input.extension == MunchkinResourcesConstants.KLIB_EXTENSION -> input
        input.isDirectory -> {
            val hash = Integer.toHexString(input.absolutePath.hashCode())
            val output = File(tempRoot, "${input.name}-$hash.${MunchkinResourcesConstants.KLIB_EXTENSION}")
    ...
    private fun updateFunctionStack(line: String, funStack: ArrayDeque<Pair<Int, String>>) {
        ...
        val signatureMatch = MunchkinResourcesRegex.IR_SIGNATURE_BLOCK.find(trimmed)
        val label = signatureMatch?.groupValues?.get(1)
            ?: MunchkinResourcesRegex.IR_NAME.find(trimmed)?.groupValues?.get(1)
            ?: trimmed
    ...
    private fun buildCallChain(
        ...
        return buildString {
            if (fileLabel != null) {
                append(fileLabel)
                append(MunchkinResourcesConstants.CALL_CHAIN_FILE_SEPARATOR)
            }
            if (chain.isNotEmpty()) {
                append(chain.joinToString(MunchkinResourcesConstants.CALL_CHAIN_STEP_SEPARATOR))
            } else {
                append(MunchkinResourcesConstants.CALL_CHAIN_TOP_LEVEL)
            }
            append(MunchkinResourcesConstants.CALL_CHAIN_LINE_SEPARATOR)
            append(callLine)
        }
    }
    ...
    private fun isResFile(currentFile: String?): Boolean {
        ...
        return file.endsWith(MunchkinResourcesConstants.RES_FILE_IOS) ||
            file.endsWith(MunchkinResourcesConstants.RES_FILE_COMMON)
    }
        ...
        ZipOutputStream(outputFile.outputStream().buffered()).use { zip ->
            sourceDir.walkTopDown().forEach { file ->
                ...
                val entryName = if (file.isDirectory) {
                    "$relative${MunchkinResourcesConstants.PATH_SEPARATOR}"
                } else {
                    relative
                }
