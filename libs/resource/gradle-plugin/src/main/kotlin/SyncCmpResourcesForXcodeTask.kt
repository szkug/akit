...
abstract class SyncCmpResourcesForXcodeTask : DefaultTask() {
    ...
    fun sync() {
        ...
        val outputDir = if (override.isNotEmpty()) {
            ...
            val builtProductsDir = System.getenv(MunchkinResourcesConstants.ENV_BUILT_PRODUCTS_DIR)?.trim().orEmpty()
            val resourcesPath = System.getenv(MunchkinResourcesConstants.ENV_UNLOCALIZED_RESOURCES_FOLDER_PATH)?.trim().orEmpty()
            if (builtProductsDir.isNotBlank() && resourcesPath.isNotBlank()) {
                ...
                if (cocoapodsDir.isBlank()) {
                    logger.lifecycle(MunchkinResourcesMessages.syncSkipMissingEnv())
                    return
                }
                File(cocoapodsDir)
            }
        ...
        val targetDir = if (baseDir.name == MunchkinResourcesConstants.RESOURCE_ROOT_DIR) {
            baseDir
        } else {
            File(baseDir, MunchkinResourcesConstants.RESOURCE_ROOT_DIR)
        }
        if (!targetDir.exists()) {
            targetDir.mkdirs()
        }
        for (input in resources.files) {
            if (input.isDirectory) {
                val destination = if (input.name == MunchkinResourcesConstants.RESOURCE_ROOT_DIR) {
                    targetDir
                } else {
...
private fun pruneUnusedResources(
    ...
    if (used.isEmpty()) {
        MunchkinResourcesLog.info(MunchkinResourcesMessages.PRUNE_SKIP_NO_USAGE)
        return
    }
