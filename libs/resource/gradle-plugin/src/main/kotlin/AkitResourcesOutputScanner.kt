...
class OutputResourcesScanner {
    ...
    fun scan(root: File): AvailableResources {
        ...
        for (prefix in prefixDirs) {
            ...
            for (file in files) {
                ...
                when {
                    normalized.startsWith("${ResourceKind.DRAWABLE.token}${MunchkinResourcesConstants.PATH_SEPARATOR}") -> {
                        ResourceIdNormalizer.drawableIdFromFile(file)?.let { drawable += it }
                    }
                    normalized.startsWith("${ResourceKind.RAW.token}${MunchkinResourcesConstants.PATH_SEPARATOR}") -> {
                        ResourceIdNormalizer.rawIdFromFile(file)?.let { raw += it }
                    }
                    normalized.endsWith(MunchkinResourcesConstants.STRINGS_FILE) -> {
                        val entries = StringsFileCodec.parse(file.readText())
                        strings += entries.keys
                    }
                    normalized.endsWith(MunchkinResourcesConstants.STRINGS_DICT_FILE) -> {
    ...
    private fun normalizeLocalizedPath(path: String): String {
        val parts = path.split(MunchkinResourcesConstants.PATH_SEPARATOR)
        return if (parts.firstOrNull()?.endsWith(MunchkinResourcesConstants.LPROJ_SUFFIX) == true) {
            parts.drop(1).joinToString(MunchkinResourcesConstants.PATH_SEPARATOR)
        } else {
            path
        }
    }
...
object ResourceIdNormalizer {
    ...
    private fun normalizeDrawableName(raw: String): Pair<String, String> {
        ...
        val withoutScale = withoutNine.replace(MunchkinResourcesRegex.DRAWABLE_SCALE_SUFFIX, "")
    ...
    private fun sanitizeIdentifier(raw: String): String {
        ...
        if (cleaned.isEmpty()) return MunchkinResourcesConstants.GENERATED_ID_EMPTY
        val first = cleaned.first()
        return if (first == '_' || first.isLetter()) cleaned else "${MunchkinResourcesConstants.GENERATED_ID_PREFIX}$cleaned"
    }
}
...
object StringsFileCodec {
    ...
    fun parse(content: String): Map<String, String> {
        ...
        for (match in MunchkinResourcesRegex.STRINGS_ENTRY.findAll(content)) {
    ...
    fun build(entries: Map<String, String>): String {
        return buildString {
            for ((key, value) in entries) {
                ...
                append(key)
                append('"')
                append(MunchkinResourcesConstants.STRINGS_ASSIGN)
                append('"')
                append(value)
                append(MunchkinResourcesConstants.STRINGS_LINE_END)
            }
        }
    }
}
