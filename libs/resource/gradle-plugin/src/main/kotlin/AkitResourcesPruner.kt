...
class OutputResourcesPruner(
    ...
    private fun pruneFiles(prefixDir: File, keepDrawable: Set<String>, keepRaw: Set<String>) {
        if (keepDrawable.isEmpty() && keepRaw.isEmpty()) {
            logger.info(MunchkinResourcesMessages.PRUNE_SKIP_NO_DRAWABLE_RAW)
            return
        }
        ...
        for (file in files) {
            ...
            if (normalized.startsWith("${ResourceKind.DRAWABLE.token}${MunchkinResourcesConstants.PATH_SEPARATOR}")) {
                val id = ResourceIdNormalizer.drawableIdFromFile(file)
                if (id != null && keepDrawable.isNotEmpty() && id !in keepDrawable) {
                    logger.info(MunchkinResourcesMessages.prunedDrawable(file.absolutePath))
                    file.delete()
                }
            } else if (normalized.startsWith("${ResourceKind.RAW.token}${MunchkinResourcesConstants.PATH_SEPARATOR}")) {
                val id = ResourceIdNormalizer.rawIdFromFile(file)
                if (id != null && keepRaw.isNotEmpty() && id !in keepRaw) {
                    logger.info(MunchkinResourcesMessages.prunedRaw(file.absolutePath))
                    file.delete()
                }
            }
        }
    ...
    private fun pruneStrings(prefixDir: File, keepStrings: Set<String>) {
        if (keepStrings.isEmpty()) {
            logger.info(MunchkinResourcesMessages.PRUNE_SKIP_NO_STRINGS)
            return
        }
        val lprojDirs = prefixDir.listFiles().orEmpty().filter {
            it.isDirectory && it.name.endsWith(MunchkinResourcesConstants.LPROJ_SUFFIX)
        }
        for (dir in lprojDirs) {
            val file = File(dir, MunchkinResourcesConstants.STRINGS_FILE)
            ...
            for ((key, value) in entries) {
                if (key in keepStrings) {
                    ...
                    logger.info(MunchkinResourcesMessages.prunedString(file.absolutePath, key))
                }
            }
            if (kept.isEmpty()) {
                file.delete()
    ...
    private fun pruneStringsDict(prefixDir: File, keepStrings: Set<String>) {
        ...
        for (dir in lprojDirs) {
            val file = File(dir, MunchkinResourcesConstants.STRINGS_DICT_FILE)
            if (!file.exists()) continue
